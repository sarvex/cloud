...
def deploy_job(
    ...
    try:
        unused_response = (
            ml_apis.projects()
            .jobs()
            .create(parent=f"projects/{project_id}", body=request_dict)
            .execute()
        )
...
def _create_request_dict(
    ...
    # Set master config
    chief_machine_config = {"imageUri": image_uri, "acceleratorConfig": {}}
    ...
    if worker_count > 0:
        ...
        worker_machine_config = {"imageUri": image_uri, "acceleratorConfig": {}}
...
def _print_logs_info(job_id, project_id):
    ...
    print(
        f"https://console.cloud.google.com/mlengine/jobs/{job_id}?project={project_id}"
    )
    print(
        f"\nPlease access your training job logs here: https://console.cloud.google.com/logs/viewer?resource=ml_job%2Fjob_id%2F{job_id}&interval=NO_LIMIT&project={project_id}\n"
    )
...
def _generate_job_id():
    ...
    return f"tf_cloud_train_{unique_tag}"
